...
class SupplierControllerTest {
    ...
    @Test
    void toggleSupplierActiveStatus_ShouldToggleStatus() throws Exception {
        SupplierResponseDto responseDto = SupplierResponseDto.builder()
                .id(1L)
                .name("Supplier 1")
                .isActive(false)
                .build();
        when(supplierService.toggleSupplierActiveStatus(1L)).thenReturn(responseDto);

        mockMvc.perform(patch("/api/suppliers/1/toggle-active"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1L))
                .andExpect(jsonPath("$.isActive").value(false));

        verify(supplierService).toggleSupplierActiveStatus(1L);
    }
}
