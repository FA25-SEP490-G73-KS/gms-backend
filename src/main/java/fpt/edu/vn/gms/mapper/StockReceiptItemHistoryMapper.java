...
public interface StockReceiptItemHistoryMapper {
        ...
        default StockReceiptItemDetailResponse toDetailDto(StockReceiptItem item,
                        List<StockReceiptItemHistory> histories) {
                ...
                // Lấy số lượng tồn kho từ Part
                Double quantityInStock = null;
                if (item.getPurchaseRequestItem() != null
                                && item.getPurchaseRequestItem().getPart() != null) {
                        quantityInStock = item.getPurchaseRequestItem().getPart().getQuantityInStock();
                }

                return StockReceiptItemDetailResponse.builder()
                                .id(item.getId())
                                .receiptId(item.getStockReceipt() != null ? item.getStockReceipt().getReceiptId()
                                                : null)
                                .partCode(item.getPurchaseRequestItem() != null
                                                && item.getPurchaseRequestItem().getPart() != null
                                                                ? item.getPurchaseRequestItem().getPart().getSku()
                                                                : null)
                                .partName(item.getPurchaseRequestItem() != null
                                                ? item.getPurchaseRequestItem().getPartName()
                                                : null)
